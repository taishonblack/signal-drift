...
- Deduplication is owned by the server write path, not by a fuzzy timestamp uniqueness key. On submission the server looks for an existing open/recent incident matching `session_id + runtime_route_id + incident_type`; if one exists, the later client observation is recorded as corroboration (and may refine observed start/end) instead of creating a new row. Client clock skew and differing sampling intervals therefore cannot produce duplicates. Exact matching window and locking are designed with the write path in E.5A.
